...
#include "dgsh-debug.h"		/* dgsh: DPRINTF(3, ) */
#if defined (DGSH)
...
static struct dgsh_conc **dgsh_conc_fds = NULL;
static struct dgsh_conc **dgsh_proc_fds = NULL;
...
static int create_dgsh_conc __P((COMMAND *, int*, int*, struct fd_bitmap *));
#endif
     ...
     struct fd_bitmap *fds_to_close;
{
  ...
  DPRINTF(3, "pipe_in: %d, pipe_out: %d, asynchronous: %d\n",
  ...
  if (command->type == cm_subshell ||
       ...
       (pipe_out != NO_PIPE || pipe_in != NO_PIPE || asynchronous)))
    {
	 ...
	 control and call execute_command () on the command again. */
...
#if defined (DGSH)
      DPRINTF(3, "dgsh_nest_level: %d, should be compound command: %s",
      ...
      if (dgsh_nest_level >= 0 && command->type != cm_connection &&
        ...
        change_dgsh_pipes(&pipe_in, &pipe_out, command);
#endif
      DPRINTF(3, "go make_child()\n");
      paren_pid = make_child (savestring (tcmd), asynchronous);

      DPRINTF(3, "make child paren pid: %d\n", paren_pid);
	  ...
	  signal_in_progress (DEBUG_TRAP) == 0 && running_trap == 0)
	{
...
#if defined (DGSH)
	  s = user_subshell == 0 && (command->type == cm_group || command->type == cm_dgsh) && pipe_in == NO_PIPE && pipe_out == NO_PIPE && asynchronous;
#else
	  ...
	  /* run exit trap for : | ( ...; ) and ( ...; ) | : */
#if defined (DGSH)
	  s += user_subshell == 0 && (command->type == cm_group || command->type == cm_dgsh) && (pipe_in != NO_PIPE || pipe_out != NO_PIPE) && asynchronous == 0;
#else
	  s += user_subshell == 0 && command->type == cm_group && (pipe_in != NO_PIPE || pipe_out != NO_PIPE) && asynchronous == 0;
#endif

	  DPRINTF(3, "go execute_in_subshell()\n");
	  last_command_exit_value = execute_in_subshell (command, asynchronous, pipe_in, pipe_out, fds_to_close);

	  DPRINTF(3, "last_command_exit_value: %d, s: %d\n",
	  ...
	  if (asynchronous == 0)
	    {
	      ...
	      DPRINTF(3, "wait for paren_pid\n");
	      exec_result = wait_for (paren_pid);
	      DPRINTF(3, "exec_result: %d\n", exec_result);

#if defined (DGSH)
              DPRINTF(3, "Try free dgsh conc at level: %d", dgsh_nest_level);
              if (dgsh_nest_level >= 0 &&
		  !strncmp(make_command_string(command), "../dgsh_conc", 12))
		//XXX: never executed: investigate
                {
                  struct dgsh_conc *current_dgsh_conc =
			  dgsh_conc_fds[dgsh_nest_level];
                  struct dgsh_conc *current_dgsh_proc =
			  dgsh_proc_fds[dgsh_nest_level];
		  DPRINTF(3, "End conc command %s at nest level %d\n",
			make_command_string(command),
			dgsh_nest_level);
		  if (!strncmp(make_command_string(command),
					  "../dgsh_conc -o", 15))
		    {
                      free(current_dgsh_conc->outfda);
                      current_dgsh_conc->outfda = NULL;
                      free(current_dgsh_proc->outfda);
                      current_dgsh_proc->outfda = NULL;
		    }
		  else if (!strncmp(make_command_string(command),
					  "../dgsh_conc -i", 15))
		    {
                      free(current_dgsh_conc->infda);
                      current_dgsh_conc->infda = NULL;
                      free(current_dgsh_proc->infda);
                      current_dgsh_proc->infda = NULL;
		    }

		  if (current_dgsh_conc->outfda == NULL &&
		      current_dgsh_conc->infda == NULL &&
		      current_dgsh_proc->outfda == NULL &&
		      current_dgsh_proc->infda == NULL)
		    {
                      free(dgsh_conc_fds[dgsh_nest_level]);
                      free(dgsh_proc_fds[dgsh_nest_level]);
                      dgsh_nest_level--;
		    }
                }
#endif
...
#if defined (DGSH)
  DPRINTF(3, "dgsh_nest_level: %d, command: %s, command type: %d",
  ...
  if (ppath)
    {
       DPRINTF(3, "Unset dgshpath from path");
       DPRINTF(3, "dgshpath in path: %s", ppath);
       if (stdpath != NULL)
         {
           DPRINTF(3, "dgshpath not in path: %s", stdpath);
  ...
  DPRINTF(3, "command type: %d, command: %s\n", command->type,
		  make_command_string(command));
  ...
  switch (command->type)
	...
	if (already_making_children && pipe_out == NO_PIPE)
      ...
      DPRINTF(3, "Completed waiting for pid: %d", last_made_pid);
    ...
    case cm_group:
      DPRINTF(3, "cm_group case: executing_function: %d, pipe_in: %d, pipe_out: %d\n",
    ...
    case cm_dgsh:
      DPRINTF(3, "cm_dgsh case: pipe_in: %d, pipe_out: %d, dgsh_in: %d, dgsh_out: %d",
	...
	{
	  if (ignore_return && command->value.Dgsh->command)
	    command->value.Dgsh->command->flags |= CMD_IGNORE_RETURN;
	  exec_result =
	    execute_command_internal (command->value.Dgsh->command,
				      asynchronous, pipe_in, pipe_out,
				      fds_to_close);
	}
        DPRINTF(3, "dgsh command returns %d", exec_result);
      break;
#endif

    case cm_connection:
      DPRINTF(3, "cm_connection case\n");
  ...
  DPRINTF(3, "exit switch for command: %d", command->type);
  ...
  DPRINTF(3, "exec_result: %d", exec_result);
...
#  endif
  ...
  last_command_exit_value = exec_result;
  DPRINTF(3, "last command exit value: %d", last_command_exit_value);
  run_pending_traps ();
#if 0
  if (running_trap == 0)
#endif
     ...
     struct fd_bitmap *fds_to_close;
{
  ...
  DPRINTF(3, "pipe_in: %d, pipe_out: %d, should_redir_stdin: %d\n",
		  pipe_in, pipe_out, should_redir_stdin);

  invert = (command->flags & CMD_INVERT_RETURN) != 0;
  user_subshell = command->type == cm_subshell || ((command->flags & CMD_WANT_SUBSHELL) != 0);
  DPRINTF(3, "user_subshell: %d, asynchronous: %d\n", user_subshell, asynchronous);
  ...
  if (user_subshell)
	...
	subshell_environment |= SUBSHELL_COPROC;
      DPRINTF(3, "subshell_environment & SUBSHELL_PIPE = %d\n",
		      subshell_environment & SUBSHELL_PIPE);
    }
...
#if defined (DGSH)
  /* We have to create the dgsh conc in this function to take note
   * of any input/output pipes for an upcoming dgsh command
     ...
     struct fd_bitmap *fds_to_close;
{
  ...
  DPRINTF(3, "pipe_in: %d, pipe_out: %d\n", pipe_in, pipe_out);
  ...
  while (cmd && cmd->type == cm_connection &&
	...
	DPRINTF(3, "Created socketpair %d -- %d\n", fildes[0], fildes[1]);
	}
      else
#endif
        re = pipe (fildes);
	...
	cmd->value.Connection->first->flags |= CMD_IGNORE_RETURN;
      DPRINTF(3, "go execute_command_internal\n");
     ...
     struct fd_bitmap *fds_to_close;
{
  ...
  switch (command->value.Connection->connector)
...
#if defined (JOB_CONTROL) && defined (DGSH)
      if ((subshell_environment || !job_control) && !stdin_redir && dgsh_nest_level < 0)
#endif
#if defined (DGSH)
      if (!stdin_redir && dgsh_nest_level < 0)
#else /* JOB_CONTROL */
      if (!stdin_redir)
#endif
	tc->flags |= CMD_STDIN_REDIR;

      DPRINTF(3, "async command first: %s, pipe_in: %d, pipe_out: %d\n",
		      make_command_string(tc), pipe_in, pipe_out);
	...
	tc->flags &= ~CMD_STDIN_REDIR;
      ...
      if (second)
	{
          DPRINTF(3, "async command second: %s, pipe_in: %d, pipe_out: %d\n",
		      make_command_string(second), pipe_in, pipe_out);
	  if (ignore_return)
  ...
  DPRINTF(3, "return exec_result: %d", exec_result);
  return exec_result;
}
...
execute_simple_command (simple_command, pipe_in, pipe_out, async, fds_to_close)
     ...
     struct fd_bitmap *fds_to_close;
{
  DPRINTF(3, "pipe_in: %d, pipe_out: %d, async: %d\n",
  ...
  if (dofork)
    {
      ...
      if (dgsh)
        {
	  DPRINTF(3, "for simple command pipe_in: %d, pipe_out: %d, dgsh_in: %d, dgsh_out: %d, executing_function: %d", pipe_in, pipe_out, dgsh_in,
      ...
      else
	{
	  ...
	  bind_lastarg ((char *)NULL);
          DPRINTF(3, "Return %d from fork", result);
	  return (result);
	}
    }
  ...
  if ((simple_command->flags & CMD_INHIBIT_EXPANSION) == 0)
    ...
    words = copy_word_list (simple_command->words);
	  ...
	  if (words == 0)
	    {
	      DPRINTF(3, "command has no words");
	  ...
	  if (find_function (words->word->word) != 0)
	    {
	      DPRINTF(3, "Executing function.");
	      executing_function++;
	      goto dgsh_command_ready;
	    }

          command_pathname = search_for_command (words->word->word, 0);
	  DPRINTF(3, "Path to command: %s", command_pathname);
	  if (command_pathname == NULL)
	    {
	      DPRINTF(3, "Command %s cannot be found. Let bash handle it",
			      words->word->word);
	      goto dgsh_command_ready;
	    }
	  ...
	  if (strstr(command_pathname, dgshpath) ||
	      strstr(command_pathname, "/build/libexec/dgsh"))
	    {
	      if (find_shell_builtin (words->word->word) ||
	        ...
	        {
		  is_dgsh_command = 1;
		  DPRINTF(3, "Command %s is a shell or special builtin; override",
				  words->word->word);
		}
	      DPRINTF(3, "Command %s is in the dgsh path",
			    command_pathname);
	      goto dgsh_command_ready;
	  }
	  ...
	  if (fd < 0)
	    {
	      DPRINTF(3, "dgsh: unable to open file %s", command_pathname);
	      return (EXECUTION_FAILURE);
	    }
	  ...
	  if (memcmp(text, "#!", 2) == 0 && (
	    ...
	    {
	      DPRINTF(3, "Script %s invokes dgsh-wrap, --dgsh, or env dgsh",
			    command_pathname);
	      goto dgsh_command_ready;
	  ...
	  DPRINTF(3, "Command %s requires wrapping",
			command_pathname);
...
run_builtin:
  ...
  if (builtin || func)
      ...
      if (already_forked)
	{
	  ...
	  if (async)
	    {
	      DPRINTF(3, "async: CMD_STDIN_REDIR: %d, pipe_in: %d, stdin_redirects: %d\n",
  ...
  DPRINTF(3, "go execute disk command");
  ...
  if (pid == 0)
      ...
      if (command == 0)
	{
	 ...
	 type it in. */
      ...
      DPRINTF(3, "go shell_execve");
      exit (shell_execve (command, args, export_env));
      DPRINTF(3, "return from shell_execve");
    }
  else
    {
parent_return:
  ...
  if (strncmp(path->value, dgshpath, strlen(dgshpath)))
    {
      ...
      DPRINTF(3, "%s(): after prepending, path: %s",
		__func__, newpath);
    }
}
 ...
 */
static void
create_conc_structures(n, isoutput)
	int n; int isoutput;
{
  struct dgsh_conc *current_dgsh_conc = dgsh_conc_fds[dgsh_nest_level];
  struct dgsh_conc *current_dgsh_proc = dgsh_proc_fds[dgsh_nest_level];
  if (isoutput)
    {
      current_dgsh_conc->outfda = (int *)xmalloc(sizeof(int) * n);
      current_dgsh_conc->outfda_size = n;
      current_dgsh_proc->outfda = (int *)xmalloc(sizeof(int) * n);
      current_dgsh_proc->outfda_size = n;
    }
  else
    {
      current_dgsh_conc->infda = (int *)xmalloc(sizeof(int) * n);
      current_dgsh_conc->infda_size = n;
      current_dgsh_proc->infda = (int *)xmalloc(sizeof(int) * n);
      current_dgsh_proc->infda_size = n;
    }
}

/**
 * Setup socketpair endpoints for concentrators and
 * processes around it and fire the command
 */
static int
execute_conc_command(conc, n, isoutput, pipe, fds_to_close)
	COMMAND *conc; int n; int isoutput; int *pipe;
	struct fd_bitmap *fds_to_close;
{
  int sp[n][2];
  ...
  DPRINTF(3, "go execute concentrator command: %s\n",
  ...
  if (isoutput)
    { // scatter block
	   ...
	   */
          current_dgsh_conc->noinput = 1;
          current_dgsh_proc->noinput = 1;
	}
      DPRINTF(3, "noinput set to: %d", current_dgsh_proc->noinput);
      current_dgsh_conc->output_type++;
      current_dgsh_proc->output_type++;
      for (i = 0; i < n; i++)
	{
          current_dgsh_conc->outfda[i] = sp[i][1];
	  current_dgsh_proc->outfda[i] = sp[i][0];
	  DPRINTF(3, "set scatter conc %d fd to %d\n", i, sp[i][1]);
	  DPRINTF(3, "set scatter proc %d fd to %d\n", i, sp[i][0]);
	}
      re = execute_command_internal(conc, 0, *pipe,
		      DGSH_CONC_PIPES, fds_to_close);
    }
  else
    { /* gather block:
         output_type = 2 in both cases:
         1. dgsh-conc -o -n X
         2. dgsh-conc -o X
         The following block can be simplified.
       */
      if (current_dgsh_conc->output_type == 1)
        current_dgsh_conc->output_type++;
      if (current_dgsh_proc->output_type == 1)
        current_dgsh_proc->output_type++;
      for (i = 0; i < n; i++)
        {
          current_dgsh_conc->infda[i] = sp[i][0];
	  current_dgsh_proc->infda[i] = sp[i][1];
	  DPRINTF(3, "set gather conc %d fd to %d\n", i, sp[i][0]);
	  DPRINTF(3, "set gather proc %d fd to %d\n", i, sp[i][1]);
        }
      re = execute_command_internal(conc, 0, DGSH_CONC_PIPES,
			  *pipe, fds_to_close);
      DPRINTF(3, "Conc command returned %d", re);
    }
  close(*pipe);
  ...
  *pipe = NO_PIPE;
  for (i = 0; i < n; i++)
    close(sp[i][isoutput]);
  DPRINTF(3, "dgsh-conc %s (output type: %d) at level %d returned %d\n",
...
/**
 * Create concentrator command
 * If the concentrator block is to multiplex both
 * process inputs to the block and outputs from the block,
 * i.e. | {{ ... }} | , then the input concentrator
 * (the one that multiplexes outputs) is fired with
 * the -r argument to pass the message block to its origin
 * in most cases (see the dgsh-conc source for more details)
 */
static COMMAND *
	...
	char *fds; char *type; char *prog; int output; char *noinput;
{
  ...
  DPRINTF(3, "created concentrator command: %s\n", make_command_string(conc));
  return conc;
}

/**
 * Create and fire an dgsh concentrator or a pair of them
 * in case of a | {{ ... }} |
 */
static int
create_dgsh_conc (command, pipe_in, pipe_out, fds_to_close)
  ...
  if (command->type == cm_dgsh)
    {
      ...
      DPRINTF(3, "CONCENTRATOR ZONE\n");
      DPRINTF(3, "pipe_in: %d, pipe_out: %d\n", *pipe_in, *pipe_out);

      dgsh_nest_level++;
      if (!dgsh_conc_fds)
        dgsh_conc_fds = (struct dgsh_conc **)xmalloc(sizeof(struct dgsh_conc *));
      else
        dgsh_conc_fds = (struct dgsh_conc **)xrealloc(dgsh_conc_fds,
			sizeof(struct dgsh_conc *) * (dgsh_nest_level+1));
      dgsh_conc_fds[dgsh_nest_level] = (struct dgsh_conc *)xmalloc(
		      sizeof(struct dgsh_conc));
      memset(dgsh_conc_fds[dgsh_nest_level], 0, sizeof(struct dgsh_conc));

      if (!dgsh_proc_fds)
        dgsh_proc_fds = (struct dgsh_conc **)xmalloc(sizeof(struct dgsh_conc *));
      else
        dgsh_proc_fds = (struct dgsh_conc **)xrealloc(dgsh_proc_fds,
			sizeof(struct dgsh_conc *) * (dgsh_nest_level+1));
      dgsh_proc_fds[dgsh_nest_level] = (struct dgsh_conc *)xmalloc(
		      sizeof(struct dgsh_conc));
      memset(dgsh_proc_fds[dgsh_nest_level], 0, sizeof(struct dgsh_conc));

      /* Create the command in reverse; it is stored in a LIFO way */
      get_dgsh_block_comm_n(command->value.Dgsh->command, &n);
      sprintf(fds, "%d", n);
      DPRINTF(3, "%d procs in dgsh group\n", n);

      wait_el.word = alloc_word_desc();
      wait_el.word->word = wait_st;
      wait_el.redirect = 0;
      wait = make_simple_command(wait_el, (COMMAND *)NULL);
      DPRINTF(3, "Inject wait command in multipipe block");
      ...
      //TODO: the output conc command fails for DGSH_DRAW_EXIT=1 example/dir.sh
 ...
 */
static int
get_dgsh_block_comm_n (command, n)
     COMMAND *command; int *n;
{
  if (!command)
    return 0;
  DPRINTF(3, "command type: %d, command: %s, n: %d\n",
  ...
  if (command->type != cm_connection)
  ...
  else
    {
      if (command->value.Connection->connector == '|')
        (*n)--;
      get_dgsh_block_comm_n(command->value.Connection->first, n);
      get_dgsh_block_comm_n(command->value.Connection->second, n);
    }
  return 0;
}
 ...
 * dgsh concentrator.
 */
static void change_dgsh_pipes(pipe_in, pipe_out, command)
     int *pipe_in; int *pipe_out; COMMAND *command;
{
  struct dgsh_conc *dgshp = dgsh_proc_fds[dgsh_nest_level];
  DPRINTF(3, "output_type: %d, outsize: %d, insize: %d, out_index: %d,"
		  "in_index: %d, pipe_in: %d, pipe_out: %d\n",
		  dgshp->output_type, dgshp->outfda_size,
		  dgshp->infda_size, dgshp->out_index,
		  dgshp->in_index, *pipe_in, *pipe_out);
   ...
   */
  if ((dgshp->output_type == 2 && (*pipe_in == NO_PIPE ||
       *pipe_out == NO_PIPE)) ||
	// Output conc  with no input: don't change its pipes
      (dgshp->output_type == 1 && *pipe_in == NO_PIPE &&
       !dgshp->noinput) ||
      (!dgshp->output_type && *pipe_out == NO_PIPE))
    {
      DPRINTF(3, "go change pipe. command: %s, all out pipes: %d,"
		      "in pipes: %d, pipe_in: %d, pipe_out: %d\n"
		      "noinput: %d",
		      make_command_string(command),
		      dgshp->outfda_size, dgshp->infda_size,
		      *pipe_in, *pipe_out, dgshp->noinput);
      int j = 0;
      int isboth = 0;

      if (dgshp->output_type == 2)
	isboth = 1;

      if ((dgshp->output_type == 1 || isboth) && *pipe_in == NO_PIPE)
	{
	  assert(dgshp->out_index < dgshp->outfda_size);
	  *pipe_in = dgshp->outfda[dgshp->out_index];
	  dgshp->out_index++;
	}

      if ((dgshp->output_type == 0 || isboth) && *pipe_out == NO_PIPE)
	{
	  assert(dgshp->in_index < dgshp->infda_size);
	  *pipe_out = dgshp->infda[dgshp->in_index];
	  dgshp->in_index++;
	}
    }
  DPRINTF(3, "dgsh_procs: pipe_in: %d, pipe_out: %d, out_index: %d, in_index: %d\n",
		  *pipe_in, *pipe_out, dgshp->out_index, dgshp->in_index);
}
#endif
     ...
     int pipe_in, pipe_out;
{
  ...
  if (pipe_in != NO_PIPE && pipe_in != INHERITED_PIPE)
	...
	{
          pipe_in = dgsh_conc_fds[dgsh_nest_level]->infda[0];
	  conc_isoutput = 0;
	}
      DPRINTF(3, "dup-close fd %d to get stdin\n", pipe_in);
      if (dup2 (pipe_in, 0) < 0)
	dup_error (pipe_in, 0);
      if (pipe_in > 0)
	close (pipe_in);
  ...
  if (pipe_out != NO_PIPE && pipe_out != INHERITED_PIPE)
    {
      if (pipe_out == DGSH_CONC_PIPES)
	{
          pipe_out = dgsh_conc_fds[dgsh_nest_level]->outfda[0];
	  conc_isoutput = 1;
	}
      if (pipe_out != REDIRECT_BOTH)
	{
	  DPRINTF(3, "dup-close fd %d to get stdout\n", pipe_out);
  ...
  if (conc_isoutput >= 0)
    {
      int i = 0;
      struct dgsh_conc *this = dgsh_conc_fds[dgsh_nest_level];
      int *fda = NULL;
      int size = -1;
      assert(dgsh_nest_level >= 0);
      if (conc_isoutput)
	{
          fda = this->outfda;
	  size = this->outfda_size;
	}
      else
	{
          fda = this->infda;
	  size = this->infda_size;
	}
      /* Inbound, outbound, or both, the size is the same */
      for (i = 1; i < size; i++)
        {
          int fd = dup2(fda[i], i + 2);
	  DPRINTF(3, "dup-close fd %d to get %d\n", fda[i], fd);
	  if (fd < 0)
	    dup_error (fda[1], i + 2);
          close(fda[i]);
	}
